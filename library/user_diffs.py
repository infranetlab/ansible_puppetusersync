...
def run_module():
    ...
    # 3. compute lists
    target_users=[x for x in users.values() if x['gid'] in target_gids and x['uid'] in target_uid_set]
    ...
    sync_lists['users_to_delete']=[{'name': x.pw_name, 'uid': x.pw_uid} for x in
                                     curr_users if x.pw_uid in uids_to_delete]
